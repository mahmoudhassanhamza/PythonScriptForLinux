...
#define CACHE_BCOPY(cmd, blk, bofs, p, nbytes)            \
  ...
  {                                                       \
    ...
    {                                                     \
    case 1:                                               \
      *((byte_t *)p) = CACHE_BYTE(&blk->data[0], bofs);   \
      break;                                              \
    case 2:                                               \
      *((half_t *)p) = CACHE_HALF(&blk->data[0], bofs);   \
      break;                                              \
    case 4:                                               \
  ...
  {                                                       \
    ...
    {                                                     \
    case 1:                                               \
      CACHE_BYTE(&blk->data[0], bofs) = *((byte_t *)p);   \
      break;                                              \
    case 2:                                               \
      CACHE_HALF(&blk->data[0], bofs) = *((half_t *)p);   \
      break;                                              \
    case 4:                                               \
  ...
  for (bindex = 0, i = 0; i < nsets; i++)
  {
    ...
    for (j = 0; j < assoc; j++)
    {
      ...
      blk->user_data = (usize != 0
                            ? (byte_t *)calloc(usize, sizeof(byte_t))
                            : NULL);
...
void cache_config(struct cache_t *cp, /* cache instance */
                  FILE *stream)       /* output stream */
{
printf("HEEEEEEEEEEEEEREEEEEEEEEEEEEEEEEEEEEEEE");
  fprintf(stream,
  ...
  fprintf(stream,
          ...
          : cp->policy == NRU    ? "NOT RECENTLY USED"
          : cp->policy == FIFO   ? "FIFO"
                                 : (abort(), ""));
}

/* register cache stats */
void cache_reg_stats(struct cache_t *cp,     /* cache instance */
             ...
             md_addr_t *repl_addr) /* for address of replaced block */
{
  ...
  struct cache_blk_t *blk, *repl, *blk_resetting;
  int lat = 0;
  ...
  /* instead of checking for every set, so we try to catch a fast hit 
  by looking at the last access */
  ...
  if (cp->policy == NRU)
  {
    if (cp->hsize)
    {
           ...
           blk = blk->hash_next)
      {
        ...
        {
          if (blk->used_bit == 0) /* after a reset, it can be in cache while used bit =0*/
          {
            cp->sets[set].num_of_used++;
          }
          if (cp->sets[set].num_of_used == cp->assoc)
          {
            for (blk_resetting = cp->sets[set].hash[hindex]; blk_resetting; blk_resetting = blk_resetting->hash_next)
            {
              blk_resetting->used_bit = 0;
            }
            cp->sets[set].num_of_used = 1;
          }
          blk->used_bit = 1;
          goto cache_hit;
        }
    ...
    else
    {
      ...
      for (blk = cp->sets[set].way_head; blk; blk = blk->way_next)
      {
        if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
        {
          ...
          if (cp->sets[set].num_of_used == cp->assoc)
          {
            for (blk_resetting = cp->sets[set].way_head; blk_resetting; blk_resetting = blk_resetting->way_next)
            {
              blk_resetting->used_bit = 0;
            }
            cp->sets[set].num_of_used = 1;
  ...
  case NRU:
  {
    ...
    int i;
    for (i = 0; i < cp->replacement_index; i++)
    {
      repl = repl->way_next;
    }
